...
describe('public API host mapping', () => {
  it('recognises only the dedicated API host', () => {
    expect(isPublicApiHost(new URL('https://api.plugins.company.example/v1/plugins/search'))).toBe(true)
    expect(isPublicApiHost(new URL('https://plugins.company.example/api/v1/plugins/search'))).toBe(false)
  ...
  it('rewrites the public /v1 paths onto internal routes and keeps the query', () => {
    const search = rewritePublicApiUrl(new URL('https://api.plugins.company.example/v1/plugins/search?q=telegram&limit=5'))
    ...
    const health = rewritePublicApiUrl(new URL('https://api.plugins.company.example/v1/health'))
  ...
  it('exposes nothing else on the public host', () => {
    for (const path of ['/api/v1/plugins/search', '/v1/plugins', '/v1/registry', '/v1/api-keys', '/api/v1/registry', '/docs/api']) {
      expect(rewritePublicApiUrl(new URL(`https://api.plugins.company.example${path}`))).toBeNull()
    }
  })

  it('permanently redirects www to the apex host with path and query intact', () => {
    const redirect = wwwRedirect(new URL('https://www.plugins.company.example/plugins?q=x'))
    expect(redirect?.status).toBe(301)
    expect(redirect?.headers.get('Location')).toBe('https://plugins.company.example/plugins?q=x')
    expect(wwwRedirect(new URL('https://plugins.company.example/'))).toBeNull()
    expect(wwwRedirect(new URL('https://api.plugins.company.example/v1/health'))).toBeNull()
  })

  it('redirects the bare host to the docs and 404s unknown paths', async () => {
    const root = publicApiNotFound('/')
    expect(root.status).toBe(302)
    expect(root.headers.get('Location')).toBe('https://plugins.company.example/docs/api')
